...
async function seedUserRole (){
  try {
    let user = await User.findOne({
      'username':'Genesis_Pagac',
      'isActive':true,
      'isDeleted':false
    });
    let userRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (user && user.isPasswordMatch('RomRZaanPOlSlNT') && userRole){
    ...
    let admin = await User.findOne({
      'username':'Henry.Gulgowski18',
      'isActive':true,
      'isDeleted':false
    });
    let adminRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (admin && admin.isPasswordMatch('02GjeDUnnHUaXBT') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('RomRZaanPOlSlNT') ) {
      let user = new User({
        'password':'RomRZaanPOlSlNT',
        'username':'Genesis_Pagac',
    ...
    if (!admin || !admin.isPasswordMatch('02GjeDUnnHUaXBT') ) {
      let admin = new User({
        'password':'02GjeDUnnHUaXBT',
        'username':'Henry.Gulgowski18',
