...
beforeAll(async function (){
  try {
    ...
    const db = client.db('Stud_test');

    const user = db.collection('user');
    inserted_user = await user.insertOne({
      username: 'Lauren.Feest5',
      password: '88sZtb22mx9iXDi',
      email: 'Elaina24@yahoo.com',
      name: 'Stephanie Goyette',
      mobileNo: '1-656-284-6484 x614',
      role: 560,
      resetPasswordLink: {},
      loginRetryLimit: 785,
      loginReactiveTime: '2022-09-29T02:17:25.948Z',
      id: '6198d4ae34a9531074f6ba95'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'August.Gutkowski',
        'password':'6sX3bC6tv_eAaLQ',
        'email':'Donato.Ullrich@gmail.com',
        'name':'Cecil Conroy',
        'mobileNo':'(271) 812-1669 x050',
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Donato.Ullrich@gmail.com', });
